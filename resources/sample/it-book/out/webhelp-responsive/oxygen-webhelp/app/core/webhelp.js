define(['util', 'jquery.highlight'], function(util) {
    ...
    $(document).ready(function () {
        ...
        //$("#wh_topic_body").css("visibility", "visible");
...
function handleSideTocPosition(scrollPosition) {
    ...
    if ($slideSection.length > 0) {
        ...
        var tocWidth = parseInt($sideToc.outerWidth());
...
function pageTocHighlightNode(scrollPosition) {
    ...
    $.each($('.wh_topic_content .topic'), function () {
        ...
        if( ($(this).offset().top - topOffset) <  $(window).scrollTop() && ( $(this).offset().top >  $(window).scrollTop()) ){
            ...
            $('#wh_topic_toc li a[data-tocid = "'+ $(this).attr('id') + '"]').addClass('current_node');
        }
    });
    return $(window).scrollTop();
}
...
function handlePageTocPosition(scrollPosition) {
    ...
    // Check if exists nodes in the Page TOC section
    // If not will increase the content section
    if(!$pageTOC.find('li').length > 0){
        $('#wh_topic_body').removeAttr('class').attr('class','col-lg-9 col-md-9 col-sm-7 col-xs-12');
    } 
    ...
    if($pageTOC.length > 0){
        ...
        if ($slideSection.length > 0) {
            ...
            var tocWidth =  parseInt($pageTOC.outerWidth());
